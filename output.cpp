...
void printgrid(const char *title, int *inpoel, const int nelem, const int npoin, double *x, double *y) {
    ...
    fprintf(fout, "ZONE T=\"fezone\", N=%d, E=%d, DATAPACKING=POINT, ZONETYPE=FETRIANGLE\n", npoin, nelem);
    ...
    for (int i=0; i<nelem; i++) {
        fprintf(fout, "%d, %d, %d, %d\n", inpoel[iu(i,0,nelem)], inpoel[iu(i,1,nelem)], inpoel[iu(i,2,nelem)], inpoel[iu(i,3,nelem)]);
    }
    fclose(fout);
}
...
void printscalar(const char *title, const char *varname, const char *varname2, const char *varname3,
    ...
    fprintf(fout, "ZONE T=\"fezone\", N=%d, E=%d, DATAPACKING=BLOCK, ZONETYPE=FETRIANGLE, varlocation=([1,2]=nodal,[3,4,5,6,7]=cellcentered)\n", npoin, nelem);
    ...
    //var 2
    for (int i = 0; i<nelem; i++) {
        fprintf(fout, "%lf, ", unkel[iu3(i, 0, 1, 1)]/unkel[iu3(i, 0, 0, 1)]);
        if ((i+1) % 200 == 0) {fprintf(fout,"\n");}
    }
    fprintf(fout, "\n\n");
    ...
    //var 3
    for (int i = 0; i<nelem; i++) {
        fprintf(fout, "%lf, ", unkel[iu3(i, 0, 2, 1)]/unkel[iu3(i, 0, 0, 1)]);
        if ((i+1) % 200 == 0) {fprintf(fout,"\n");}
    }
    fprintf(fout, "\n\n");
    ...
    //var 4
    for (int i = 0; i<nelem; i++) {
        fprintf(fout, "%lf, ", unkel[iu3(i, 0, 3, 1)]/unkel[iu3(i, 0, 0, 1)]);
        if ((i+1) % 200 == 0) {fprintf(fout,"\n");}
    }
    fprintf(fout, "\n\n");
